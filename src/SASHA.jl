...
"""
    ...
    the fitted model. Should be implemented for any custom model type before using the
    SASHA optimization functions.
...
"""
    ...
    Evaluates the loss of `model` on `data`. Should be implemented for any custom model
    type before using the SASHA optimization functions.
...
"""
    ...
    splatted before the respective function calls. Returns a tuple with the winning arm
    and its final loss.
...
"""
    ...
    tuples, the arguments are splatted before the respective function calls. Returns a
    tuple with the winning arm and its final loss.
